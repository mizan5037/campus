        ...
        <div class="container">
            <div class="row">
                <div class="col-md-8">
                    ...
                    <div class="page">
                     <h3></h3>
                       @foreach($tuitionposts as $tuitionpost)
                       <div class="details" style="background-color: #B1B1B1;">
                            <ul>
                                <li>Posted By: {{Auth::user()->name}}<a href="" ></a>  </li>
                                <li>Published:{{date('M j,Y',strtotime($tuitionpost->created_at))}}</li>
                        ...
                        <div class="content">
                         <p>location:{{$tuitionpost->location}} <br><br>
                            Category:{{$tuitionpost->bangla}} <br><br>
                            Salary:{{$tuitionpost->salary}} <br><br>
                            Gender:{{$tuitionpost->location}} <br><br>
                            Contact:{{$tuitionpost->contact}} <br><br>
                   ...
                   <button type="button" class="btn btn-info btn-lg" data-toggle="modal" data-target="#myModal">Tuition Create</button>
							    ...
							    <div class="form-group">
							       {!!Form::label('bangla','Bangla Medium:')!!}
							           <select class="form-control" name="bangla">
							             @foreach($tuitioncategories as $category)
							               <option value="{{$category->id}}">{{$category->bangla}}</option>
							             @endforeach
							           </select>
							    </div>
                    ...
                    <div class="widgets" style="background-color:gray;">
                        ...
                        <ul style="color:green;">
                         @foreach($tuitioncategories as $category)
                            <li><a href="{{route('tuition.show',$category->id)}}" >{{$category->bangla}}</a></li>
                         @endforeach
